...
struct CircularView: View {
    ...
    @State private var endProgress = ArrowAnimConstant.startEndProgress
    ...
    @State private var startProgress = ArrowAnimConstant.startEndProgress
    
    // State manage for animation completed
    @State var isAnimationCompleted = false
    ...
    var arrowAnimStyle: ArrowAnimStyle
    
    // Animation duration
    var animationDuration = ArrowAnimConstant.animationDuration
        ...
        ZStack {
            ...
            Circle()
                .stroke(
                    ...
                    .opacity(ArrowAnimConstant.circleStorkeOpacity),
            ...
            animatedCircle()
                .rotation3DEffect(
                    .degrees(ArrowAnimConstant.oppositeDirectionAngle), axis: (x: 1, y: .zero, z: .zero)
                )
            
            // Left arrow with animation
            Arrow()
                .stroke(style: StrokeStyle(lineWidth: ArrowAnimConstant.lineWidth, lineCap: .round, lineJoin: .round))
                .frame(
    ...
    private func animateView() {
        isArrowClicked = true
        startProgress = ArrowAnimConstant.halfProgress
    ...
    private func adjustArrowSize() {
        if arrowSize < arrowAnimStyle.circleSize / 4 {
            ...
            arrowSize = arrowAnimStyle.circleSize - 15
        }
    }
}
